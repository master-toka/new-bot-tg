...
from config import BOT_TOKEN, LOG_FORMAT, LOG_LEVEL, ADMIN_ID, GROUP_ID, DISTRICTS
...
async def on_startup(bot: Bot):
    ...
    try:
        ...
        if chat_member.status not in ['administrator', 'creator']:
            logger.warning(f"⚠️ Бот не является администратором группы {GROUP_ID}")
            logger.warning("Для редактирования сообщений и удаления кнопок нужны права администратора")
        else:
            logger.info(f"✅ Бот является администратором группы {GROUP_ID}")
    except Exception as e:
    ...
    try:
        await bot.send_message(
            ...
            f"🏘 Районов в системе: {len(DISTRICTS)}\n\n"
...
async def main():
    ...
    # ВАЖНО: Правильный порядок регистрации роутеров!
    # Сначала регистрируем специфичные роутеры (с конкретными фильтрами)
    dp.include_router(customer.router)    # Хендлеры для заказчиков
    dp.include_router(installer.router)   # Хендлеры для монтажников (включая callback'и take_ и refuse_)
    dp.include_router(admin.router)       # Хендлеры для админа
    
    # ПОТОМ регистрируем общие роутеры
    dp.include_router(group.router)       # Общие хендлеры для группы (только сообщения)
    dp.include_router(common.router)      # Общие хендлеры (start, help)
